...
- **Next: Step 4** — de-annualize: remove the `'Annual target'` label + widget (`statement.js:51`, `index.html:496,718–719`), reframe achievement as monthly with a "Monthly · <period>" cadence badge, add a UI parity test (displayed incentive == contract `final`), and make `elevate_contract_v3` the default once green.
